...
class SearchService:
    def search(
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["person", "persons"]:
            ...
            for p in persons:
                mtype = "exact" if p.id == q or p.name.lower() == q.lower() else "partial"
            ...
            for p in all_persons:
                if p.id not in seen_ids:
                    if p.aliases and any(q.lower() in str(alias).lower() for alias in p.aliases):
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["phone", "phones"]:
            phones = db.query(Phone).filter(
                or_(Phone.id.ilike(f"%{q}%"), Phone.msisdn.ilike(f"%{q}%"), Phone.registered_name.ilike(f"%{q}%"))
            ).limit(limit).all()
            for ph in phones:
                mtype = "exact" if ph.id == q or ph.msisdn == q else "partial"
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["sim", "sims"]:
            sims = db.query(SIM).filter(
                or_(SIM.id.ilike(f"%{q}%"), SIM.iccid.ilike(f"%{q}%"), SIM.imsi.ilike(f"%{q}%"))
            ).limit(limit).all()
            for s in sims:
                mtype = "exact" if s.id == q or s.iccid == q else "partial"
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["device", "devices"]:
            devices = db.query(Device).filter(
                or_(Device.id.ilike(f"%{q}%"), Device.imei.ilike(f"%{q}%"), Device.model.ilike(f"%{q}%"))
            ).limit(limit).all()
            for d in devices:
                mtype = "exact" if d.id == q or d.imei == q else "partial"
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["vehicle", "vehicles"]:
            vehicles = db.query(Vehicle).filter(
                or_(Vehicle.id.ilike(f"%{q}%"), Vehicle.registration_number.ilike(f"%{q}%"))
            ).limit(limit).all()
            for v in vehicles:
                mtype = "exact" if v.id == q or v.registration_number == q else "partial"
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["bankaccount", "account", "accounts"]:
            accounts = db.query(BankAccount).filter(
                or_(BankAccount.id.ilike(f"%{q}%"), BankAccount.account_number.ilike(f"%{q}%"))
            ).limit(limit).all()
            for a in accounts:
                mtype = "exact" if a.id == q or a.account_number == q else "partial"
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["location", "locations"]:
            locs = db.query(Location).filter(
                or_(Location.id.ilike(f"%{q}%"), Location.name.ilike(f"%{q}%"), Location.police_station.ilike(f"%{q}%"))
            ).limit(limit).all()
            for loc in locs:
                mtype = "exact" if loc.id == q or loc.name.lower() == q.lower() else "partial"
        ...
        if not entity_type_filter or entity_type_filter.lower() in ["fir", "firs"]:
            firs = db.query(FIR).filter(
                or_(FIR.id.ilike(f"%{q}%"), FIR.fir_number.ilike(f"%{q}%"), FIR.summary.ilike(f"%{q}%"))
            ).limit(limit).all()
            for fir in firs:
                mtype = "exact" if fir.id == q or fir.fir_number == q else "partial"
